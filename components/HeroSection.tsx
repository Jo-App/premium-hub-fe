...
const slides = [
  {
    ...
    sub: "한적하고, 쾌적한 부산 중심 300 평가 기간 수도 거주\n부산 최고의 입지에서 새로운 삶의 방식과 국가를 경험하세요",
  },
  {
    bg: "from-sky-900/70 via-sky-800/60 to-sky-900/70",
    heading: (
      <>
        배운대를 품은
        <br />
    ...
    sub: "프리미엄 커뮤니티와 최상의 주거 환경을 경험하세요",
  },
];
...
export default function HeroSection() {
  ...
  return (
    <section className="relative w-full h-[480px] md:h-[600px] lg:h-[680px] overflow-hidden">
      {/* Background — placeholder gradient (replace with real image) */}
      <div
        className={`absolute inset-0 bg-gradient-to-br ${slides[current].bg} bg-slate-800 transition-all duration-700`}
      >
        {/* Simulated city skyline overlay */}
      ...
      <div className="absolute top-[68px] md:top-20 left-0 right-0 z-10 flex justify-center md:justify-start md:pl-[calc(50%-280px)]">
        <div className="bg-black/40 text-white text-xs px-4 py-1.5 rounded-full backdrop-blur-sm">
          📣 현재는 <span className="text-[var(--brand)] font-semibold">사전 알림 신청 기간</span>입니다
        </div>
      </div>
      ...
      <div className="relative z-10 h-full flex items-center">
        <div className="max-w-6xl mx-auto px-6 w-full">
          <div className="max-w-xl">
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-snug drop-shadow-md">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              <a
                href="#contact"
                className="text-sm font-semibold border-2 border-white text-white px-5 py-3 rounded hover:bg-white/10 transition-colors"
              >
                견학신청하기
              </a>
            </div>
        ...
        {slides.map((_, i) => (
          <button
            ...
            className={`w-2 h-2 rounded-full transition-all ${
              i === current ? "bg-[var(--brand)] w-5" : "bg-white/50"
            }`}
            aria-label={`슬라이드 ${i + 1}`}
          />
        ))}
