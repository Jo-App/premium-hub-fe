import { getHomeFeatures } from "@/app/lib/api";
...
export default async function FeaturesSection() {
  const features = await getHomeFeatures().catch(() => []);
    ...
    <section id="features" className="bg-white py-14 md:py-20">
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-6">
          {features.map((f) => (
            <div key={f.id} className="flex flex-col items-center text-center gap-4 p-6 rounded-xl hover:shadow-md transition-shadow">
              <div className="w-16 h-16 rounded-full bg-[var(--brand-light)] text-[var(--brand)] flex items-center justify-center shrink-0">
                <FeatureIcon iconKey={f.icon_key} />
              </div>
              <div>
